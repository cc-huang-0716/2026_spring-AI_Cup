...
VERSION_TAG = "CCHUANG_V3_3_BUSINESS_NOPLAYER_MATCHSPLIT_ENSEMBLE_20260514"
...
class RallyDataset(Dataset):
    ...
    def _build_samples(self):
        ...
        for _, group in self.df.groupby('rally_uid'):
            ...
            player_ids = group['gamePlayerId'].values if USE_PLAYER_FEATURES else np.zeros_like(group['gamePlayerId'].values)
            other_player_ids = group['gamePlayerOtherId'].values if USE_PLAYER_FEATURES else np.zeros_like(group['gamePlayerOtherId'].values)
...
def run_group_kfold(
    ...
    return fold_scores, best_fold, best_threshold, fold_thresholds
...
def generate_submission_ensemble(
    ...
    print("推論：5-fold ensemble")

    models = []
    for model_path in model_paths:
        if not os.path.exists(model_path):
            print(f"[Warning] 找不到模型，略過: {model_path}")
            continue
    ...
    if not models:
        raise FileNotFoundError("沒有可用的 fold model 可做 ensemble")
    ...
    with torch.no_grad():
        for (
            ...
            sex, match_id, number_game, rally_id, player_id, other_player_id,
            rally_uids
        ) in tqdm(loader, desc="Inference ensemble"):
            act = act.to(device)
            pos = pos.to(device)
            spin = spin.to(device)
            strength = strength.to(device)
            ...
            for model in models:
                ...
                action_prob = torch.softmax(l_act, dim=-1)
                point_prob = torch.softmax(l_pos, dim=-1)
                win_prob = torch.sigmoid(l_win).squeeze(-1)

                action_prob_sum = action_prob if action_prob_sum is None else action_prob_sum + action_prob
            ...
            action_prob_avg = action_prob_sum / len(models)
            point_prob_avg = point_prob_sum / len(models)
            win_prob_avg = win_prob_sum / len(models)

            pred_action = torch.argmax(action_prob_avg, dim=-1).cpu().numpy()
    ...
    sub_df = pd.DataFrame(results)
    out_path = 'data/submission_test_new_publiclike_v3_3_noplayer_matchsplit_ensemble.csv'
...
if __name__ == "__main__":
    ...
    fold_scores, best_fold, best_threshold, fold_thresholds = run_group_kfold(
        train_df=df_train,
        num_actions=NUM_ACTIONS,
        num_positions=NUM_POSITIONS,
        num_points=NUM_POINTS,
    ...
    model_paths = [f"best_model_publiclike_diag_fold{i}.pth" for i in range(1, len(fold_scores) + 1)]
    ensemble_threshold = float(np.mean(fold_thresholds)) if fold_thresholds else best_threshold
    print("使用模型 ensemble:", model_paths)
    print("使用 serverGetPoint ensemble threshold:", ensemble_threshold)
    ...
    generate_submission_ensemble(
        ...
        win_threshold=ensemble_threshold
    )
