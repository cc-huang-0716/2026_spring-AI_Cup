...
def compute_class_weights(df, col_name, num_classes, max_weight=5.0):
...
def build_transition_log_probs(df, from_col, to_col, num_from, num_to, alpha=0.5):

    counts = np.full((num_from, num_to), alpha, dtype=np.float32)

    for _, group in df.groupby("rally_uid"):
        group = group.sort_values("strikeNumber").reset_index(drop=True)

        prev_vals = group[from_col].values[:-1]
        next_vals = group[to_col].values[1:]

        for prev_v, next_v in zip(prev_vals, next_vals):
            if 0 <= prev_v < num_from and 0 <= next_v < num_to:
                counts[int(prev_v), int(next_v)] += 1.0
...
def build_id_maps(train_df, id_cols=ID_COLS):
    id_maps = {}

    for col in id_cols:
        values = sorted(train_df[col].dropna().unique().tolist())
        id_maps[col] = {v: i + 1 for i, v in enumerate(values)}

    all_players = (
        set(train_df["gamePlayerId"].dropna().unique().tolist())
        | set(train_df["gamePlayerOtherId"].dropna().unique().tolist())
    )
...
class MambaTransformer(nn.Module):
    ...
    def masked_pooling(self, memory, pad_mask):
        ...
        last_idx = lengths - 1
        batch_idx = torch.arange(b, device=memory.device)
        last_h = memory[batch_idx, last_idx]

        valid_mask_float = valid_mask.unsqueeze(-1).float()
        mean_h = (memory * valid_mask_float).sum(dim=1) / lengths.unsqueeze(-1).float()

        memory_for_max = memory.masked_fill(~valid_mask.unsqueeze(-1), -1e9)
        max_h = memory_for_max.max(dim=1).values

        feat = torch.cat([last_h, mean_h, max_h], dim=-1)
    ...
    def forward(
        ...
        feat = self.masked_pooling(memory, pad_mask)
...
class CompetitionSequenceLoss(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "action_weight",
            action_weight if action_weight is not None else None
        )
        self.register_buffer(
            "point_weight",
            point_weight if point_weight is not None else None
        )
        self.register_buffer(
            "win_pos_weight",
            win_pos_weight if win_pos_weight is not None else None
        )
    ...
    def forward(self, preds, targets, winner_weight=None):
        ...
        l_point = F.cross_entropy(
            point_p,
            point_t,
            weight=self.point_weight,
            label_smoothing=self.label_smoothing
        )
...
class RallyDataset(Dataset):
    ...
    def _build_samples(self):
        ...
        for _, group in self.df.groupby('rally_uid'):
            ...
            matches = group['match_idx'].values
            number_games = group['numberGame_idx'].values
            rally_ids = group['rally_id_idx'].values
            player_ids = group['gamePlayerId_idx'].values
            other_player_ids = group['gamePlayerOtherId_idx'].values
            ...
            for t in range(1, len(group)):
                ...
                progress = t / num_prefix_samples
                winner_weight = (progress ** gamma) / num_prefix_samples
...
def train_one_fold(
    ...
    action_weights = compute_class_weights(
        df_train,
        "actionId",
        num_actions
    ).to(device)

    point_weights = compute_class_weights(
        df_train,
        "pointId",
        num_points
    ).to(device)

    print("action weights:", action_weights.detach().cpu().numpy())
    print("point weights:", point_weights.detach().cpu().numpy())
    ...
    best_model_path = f"best_model_v2_fold{fold_id}.pth"
    ...
    for epoch in range(num_epochs):
        ...
        if total_score > best_score:
            ...
            print(f"Fold {fold_id} best score {best_score:.4f}，儲存 {best_model_path}")
...
def generate_submission_ensemble(
    ...
    max_rally_len,
    model_paths,
    action_transition_log_probs=None,
    point_transition_log_probs=None,
    beta_action=0.2,
    beta_point=0.2,
    output_path=None
):
    ...
    if len(model_paths) == 0:
        raise FileNotFoundError("找不到任何 best_model_fold*.pth")
    ...
    with torch.no_grad():
        for (
            ...
            sex, match_id, number_game, rally_id, player_id, other_player_id,
            rally_uids
        ) in tqdm(loader, desc="Ensemble Inference"):
            ...
            sum_action_logits = None
            sum_point_logits = None
            sum_win_logits = None

            for model in models:
                l_act, l_point, l_win = model(
                    act,
                    pos,
                    spin,
                    strength,
                ...
                if sum_action_logits is None:
                    sum_action_logits = l_act
                    sum_point_logits = l_point
                    sum_win_logits = l_win
                else:
                    sum_action_logits += l_act
                    sum_point_logits += l_point
                    sum_win_logits += l_win

            avg_action_logits = sum_action_logits / len(models)
            avg_point_logits = sum_point_logits / len(models)
            avg_win_logits = sum_win_logits / len(models)

            if action_transition_log_probs is not None:
                action_transition_log_probs = action_transition_log_probs.to(device)

                lengths = (act != PAD_IDX).sum(dim=1).clamp(min=1)
                last_idx = lengths - 1
                batch_idx = torch.arange(act.size(0), device=device)

                last_action = act[batch_idx, last_idx]
                last_action = torch.clamp(last_action, min=0, max=num_actions - 1)

                trans_action_logits = action_transition_log_probs[last_action]
                avg_action_logits = avg_action_logits + beta_action * trans_action_logits

            if point_transition_log_probs is not None:
                point_transition_log_probs = point_transition_log_probs.to(device)

                lengths = (point != PAD_IDX).sum(dim=1).clamp(min=1)
                last_idx = lengths - 1
                batch_idx = torch.arange(point.size(0), device=device)

                last_point = point[batch_idx, last_idx]
                last_point = torch.clamp(last_point, min=0, max=num_points - 1)

                trans_point_logits = point_transition_log_probs[last_point]
                avg_point_logits = avg_point_logits + beta_point * trans_point_logits

            pred_action = torch.argmax(avg_action_logits, dim=-1).cpu().numpy()
            pred_point = torch.argmax(avg_point_logits, dim=-1).cpu().numpy()

            pred_win_prob = torch.sigmoid(avg_win_logits).squeeze(-1).cpu().numpy()
    ...
    assert sub_df["rally_uid"].is_unique
    assert len(sub_df) == test_df["rally_uid"].nunique()
    assert sub_df["actionId"].between(0, num_actions - 1).all()
    assert sub_df["pointId"].between(0, num_points - 1).all()
    assert sub_df["serverGetPoint"].isin([0, 1]).all()
    ...
    sub_df.to_csv(output_path, index=False, encoding="utf-8", lineterminator="\n")
    print(f"生成 {output_path.resolve()}")
...
if __name__ == "__main__":
    ...
    test_csv = 'data/test_new.csv'

    if os.path.exists(train_csv):
        df_train = pd.read_csv(train_csv)

        id_maps = build_id_maps(df_train)
        df_train = apply_id_maps(df_train, id_maps)

        NUM_ACTIONS = int(df_train['actionId'].max()) + 1
        NUM_POSITIONS = int(df_train['positionId'].max()) + 1
        NUM_POINTS = int(df_train['pointId'].max()) + 1

        action_transition_log_probs = build_transition_log_probs(
            df_train,
            from_col="actionId",
            to_col="actionId",
            num_from=NUM_ACTIONS,
            num_to=NUM_ACTIONS,
            alpha=0.5
        )

        point_transition_log_probs = build_transition_log_probs(
            df_train,
            from_col="pointId",
            to_col="pointId",
            num_from=NUM_POINTS,
        ...
        NUM_MATCHES = len(id_maps["match"]) + 1
        NUM_NUMBER_GAMES = len(id_maps["numberGame"]) + 1
        NUM_RALLY_IDS = len(id_maps["rally_id"]) + 1

        NUM_PLAYERS = len(id_maps["gamePlayerId"]) + 1
        MAX_LEN = int(df_train['strikeNumber'].max()) + 20

        fold_scores, best_fold = run_group_kfold(
            train_df=df_train,
            num_actions=NUM_ACTIONS,
            num_positions=NUM_POSITIONS,
            num_points=NUM_POINTS,
        ...
        if os.path.exists(test_csv):
            ...
            df_test = apply_id_maps(df_test, id_maps)

            print("ID encoding check - test:")
            for col in ["match", "numberGame", "rally_id", "gamePlayerId", "gamePlayerOtherId"]:
                idx_col = f"{col}_idx"
                print(
                    col,
                    "idx max:",
                    df_test[idx_col].max(),
                    "UNK count:",
                    (df_test[idx_col] == 0).sum()
                )

            model_paths = [
                f"best_model_v2_fold{i}.pth"
                for i in range(1, 6)
                if os.path.exists(f"best_model_v2_fold{i}.pth")
            ]

            if len(model_paths) == 0:
                raise FileNotFoundError("找不到任何 best_model_v2_fold*.pth")
            ...
            generate_submission_ensemble(
                ...
                action_transition_log_probs=action_transition_log_probs,
                point_transition_log_probs=point_transition_log_probs,
                beta_action=0.2,
                beta_point=0.2
            )
        else:
