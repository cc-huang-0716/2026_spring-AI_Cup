...
def train_ag_oof(
    ...
    for fold, (tr_idx, va_idx) in enumerate(split_iter):
        ...
        if resume_ag_folds and os.path.isdir(fold_path):
            try:
                print(f"[AG-RESUME] trying to reuse existing fold: {fold_path}")
                predictor = TabularPredictor.load(fold_path)
                ...
                try:
                    ...
                    print(f"[AG-MODELS][WARN] audit failed for resumed {name} fold={fold}: {e}")
                fold_infos.append({"name": name, "fold": fold, "n_train": int(len(tr_idx)), "n_valid": int(len(va_idx)), "macro_f1_indexed": None if score is None else float(score), "resumed": True})
                print(f"[AG-RESUME] {name} fold={fold} reused score(indexed)={score}")
                continue
            except Exception as e:
                print(f"[AG-RESUME][WARN] existing fold unusable: {fold_path} ({repr(e)}). Retraining this fold.")
...
def train_ag_oof_sweep(
    ...
    if len(metrics) == 1:
        return train_ag_oof(train_feat, test_feat, label_col, classes, out_dir, groups, folds, presets, time_limit, eval_metric, no_catboost, seed, name, model_suite=model_suite, fit_strategy=fit_strategy, ag_num_gpus=ag_num_gpus, ag_max_memory_usage_ratio=ag_max_memory_usage_ratio, resume_ag_folds=resume_ag_folds)
    ...
    for m in metrics:
        ...
        try:
            oof, test, infos = train_ag_oof(
                ...
                time_limit, m, no_catboost, seed, run_name, model_suite=model_suite, fit_strategy=fit_strategy, ag_num_gpus=ag_num_gpus, ag_max_memory_usage_ratio=ag_max_memory_usage_ratio, resume_ag_folds=resume_ag_folds,
            )
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    action_oof_small, action_test_small, infos = train_ag_oof_sweep(
        ...
        fit_strategy=args.fit_strategy, ag_num_gpus=args.ag_num_gpus, ag_max_memory_usage_ratio=args.ag_max_memory_usage_ratio, resume_ag_folds=args.resume_ag_folds,
    )
    task_infos += infos
    if args.action_0_14:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--resume_ag_folds", action="store_true", help="Reuse completed AutoGluon fold folders if loadable; retrain missing/corrupted folds.")
