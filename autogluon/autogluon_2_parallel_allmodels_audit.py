...
def train_ag_oof(
    ...
    for fold, (tr_idx, va_idx) in enumerate(split_iter):
        ...
        requested_hp = get_ag_hyperparameters(no_catboost=no_catboost, model_suite=model_suite)
        ...
        fit_kwargs = dict(
            ...
            ag_args_fit={"num_gpus": int(ag_num_gpus)},
...
def train_ag_oof_sweep(
    ...
    if len(metrics) == 1:
        return train_ag_oof(train_feat, test_feat, label_col, classes, out_dir, groups, folds, presets, time_limit, eval_metric, no_catboost, seed, name, model_suite=model_suite, fit_strategy=fit_strategy, ag_num_gpus=ag_num_gpus)
    ...
    for m in metrics:
        ...
        try:
            oof, test, infos = train_ag_oof(
                ...
                time_limit, m, no_catboost, seed, run_name, model_suite=model_suite, fit_strategy=fit_strategy, ag_num_gpus=ag_num_gpus,
            )
    ...
    if not oofs:
        ...
        return train_ag_oof(train_feat, test_feat, label_col, classes, out_dir, groups, folds, presets, time_limit, eval_metric, no_catboost, seed, name, model_suite="fast", fit_strategy=fit_strategy, ag_num_gpus=ag_num_gpus)
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    action_oof_small, action_test_small, infos = train_ag_oof_sweep(
        ...
        fit_strategy=args.fit_strategy, ag_num_gpus=args.ag_num_gpus,
    )
    task_infos += infos
    if args.action_0_14:
