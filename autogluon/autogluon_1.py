...
def add_hmm_state_features(
    ...
    try:
        ...
        print(f"[HMM] hmmlearn unavailable ({e}); filling HMM features with zeros. Install: pip install hmmlearn")
        train["hmm_state"] = 0
        test["hmm_state"] = 0
    ...
    if len(feat_cols) < 2 or len(train) < max(n_states * 5, 20):
        print("[HMM] not enough usable numeric state features; filling zeros.")
        train["hmm_state"] = 0
        test["hmm_state"] = 0
        for i in range(n_states):
            train[f"hmm_prob_{i}"] = 0.0
    ...
    train["_hmm_orig_order"] = train_order
    if "rally_uid" in train.columns:
        ...
        lengths = sorted_train.groupby("rally_uid", sort=False).size().astype(int).tolist()
    else:
        sorted_train = train.copy().reset_index(drop=True)
        lengths = [len(sorted_train)]

    scaler = StandardScaler()
    X_sorted = scaler.fit_transform(sorted_train[feat_cols].replace([np.inf, -np.inf], np.nan).fillna(0.0).astype(np.float32))
    ...
    try:
        ...
        model.fit(X_sorted, lengths=lengths)
        state_sorted = model.predict(X_sorted, lengths=lengths).astype(np.int64)
        prob_sorted = model.predict_proba(X_sorted, lengths=lengths).astype(np.float32)
        state_test = model.predict(X_test).astype(np.int64)
        prob_test = model.predict_proba(X_test).astype(np.float32)
    except Exception as e:
        print(f"[HMM] GaussianHMM failed ({e}); falling back to KMeans-like zero HMM features.")
        train = train.drop(columns=["_hmm_orig_order"], errors="ignore")
        train["hmm_state"] = 0
        test["hmm_state"] = 0
        for i in range(n_states):
            train[f"hmm_prob_{i}"] = 0.0
    ...
    print(f"[HMM] added GaussianHMM states: n_states={n_states}, features={len(feat_cols)}")
    return train, test
...
def get_ag_hyperparameters(no_catboost: bool = True, model_suite: str = "fast") -> Dict[str, Any]:
...
def train_ag_oof(
    ...
    for fold, (tr_idx, va_idx) in enumerate(split_iter):
        ...
        predictor.fit(
            ...
            hyperparameters=get_ag_hyperparameters(no_catboost=no_catboost, model_suite=model_suite),
...
def get_metric_sweep_list(label_col: str, classes: Sequence[int], base_metric: str, metric_suite: str) -> List[str]:
    ...
    if len(classes) == 2:
        ...
        metrics = ["f1_macro", "log_loss", "balanced_accuracy", "accuracy"]
    out = []
...
def train_ag_oof_sweep(
    ...
    if len(metrics) == 1:
        return train_ag_oof(train_feat, test_feat, label_col, classes, out_dir, groups, folds, presets, time_limit, eval_metric, no_catboost, seed, name, model_suite=model_suite)

    print(f"[AG-SWEEP] {name}: model_suite={model_suite}, metrics={metrics}, time_limit_each={time_limit}")
    ...
    for m in metrics:
        ...
        try:
            oof, test, infos = train_ag_oof(
                ...
                time_limit, m, no_catboost, seed, run_name, model_suite=model_suite,
            )
    ...
    if not oofs:
        ...
        return train_ag_oof(train_feat, test_feat, label_col, classes, out_dir, groups, folds, presets, time_limit, eval_metric, no_catboost, seed, name, model_suite="fast")
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    if args.use_action_aux:
        ...
        grp_oof, grp_test, infos = train_ag_oof_sweep(train_feat, test_feat, "target_aid_group", list(range(NUM_AID_GROUP_CLASSES)), os.path.join(args.out_dir, "ag_models"), groups, args.folds, args.ag_presets, max(60, args.ag_time_limit // 3), "f1_macro", args.ag_no_catboost, args.seed, "aid_group_aux", model_suite=args.ag_model_suite, metric_suite=args.ag_metric_suite)
        sub_oof, sub_test, infos2 = train_ag_oof_sweep(train_feat, test_feat, "target_aid_sub", list(range(NUM_AID_SUB_CLASSES)), os.path.join(args.out_dir, "ag_models"), groups, args.folds, args.ag_presets, max(60, args.ag_time_limit // 3), "f1_macro", args.ag_no_catboost, args.seed, "aid_sub_aux", model_suite=args.ag_model_suite, metric_suite=args.ag_metric_suite)
    ...
    if args.use_point_hier:
        ...
        valid_oof, valid_test, infos = train_ag_oof_sweep(train_feat, test_feat, "target_point_valid", [0, 1], os.path.join(args.out_dir, "ag_models"), groups, args.folds, args.ag_presets, max(60, args.ag_time_limit // 3), "f1_macro", args.ag_no_catboost, args.seed, "point_valid", model_suite=args.ag_model_suite, metric_suite=args.ag_metric_suite)
        task_infos += infos
        valid_rows = train_feat["target_point_valid"].astype(int).to_numpy() == 1
                                                                                                                                                            
        depth_oof, depth_test, infos = train_ag_oof_sweep(train_feat.loc[valid_rows].reset_index(drop=True), test_feat, "target_pid_depth", [1, 2, 3], os.path.join(args.out_dir, "ag_models"), groups[valid_rows], args.folds, args.ag_presets, max(60, args.ag_time_limit // 3), "f1_macro", args.ag_no_catboost, args.seed, "point_depth", model_suite=args.ag_model_suite, metric_suite=args.ag_metric_suite)
        ...
        task_infos += infos
        side_oof, side_test, infos = train_ag_oof_sweep(train_feat.loc[valid_rows].reset_index(drop=True), test_feat, "target_pid_side", [1, 2, 3], os.path.join(args.out_dir, "ag_models"), groups[valid_rows], args.folds, args.ag_presets, max(60, args.ag_time_limit // 3), "f1_macro", args.ag_no_catboost, args.seed, "point_side", model_suite=args.ag_model_suite, metric_suite=args.ag_metric_suite)
    ...
    if args.use_server and "target_win" in train_feat.columns:
        ...
        win_oof, win_test_prob2, infos = train_ag_oof_sweep(
            ...
            args.folds, args.ag_presets, max(120, args.ag_time_limit // 2), "roc_auc", args.ag_no_catboost, args.seed, "server_win",
            model_suite=args.ag_model_suite, metric_suite=args.ag_metric_suite,
        )
        task_infos += infos
    ...
    try:
        ...
        oof_summary.append({"target": "action", "macro_f1": float(f1_score(y_a, action_oof.argmax(axis=1), labels=list(range(NUM_ACTION_CLASSES)), average="macro", zero_division=0))})
    except Exception:
        pass
    try:
        y_p = train_feat["target_point"].astype(int).to_numpy()
        oof_summary.append({"target": "point", "macro_f1": float(f1_score(y_p, point_oof_hier.argmax(axis=1), labels=list(range(NUM_POINT_CLASSES)), average="macro", zero_division=0))})
    except Exception:
        pass
    if win_oof_score is not None:
        oof_summary.append({"target": "server", "auc": win_oof_score})
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--use_hmm_state", action="store_true")
    ...
    p.add_argument("--ag_time_limit", type=int, default=600)
    ...
    p.add_argument("--ag_model_suite", default="fast", choices=["fast", "original", "all", "everything", "all_models"], help="AutoGluon model zoo: fast=baseline; all=broad stable zoo; everything/all_models=all plus newer/foundation/experimental tabular models.")
    ...
    p.add_argument("--try_everything", action="store_true", help="Shortcut: full_open + ag_model_suite=everything + ag_metric_suite=all + CatBoost enabled.")
